...
class Document:
    ...
    def embed_candidates(
        ...
        if cand_mode == "MaskFirst" or cand_mode == "MaskAll":
            ...
            escaped_docs = [
                re.sub(re.escape(candidate), "<mask>", self.raw_text, occurences)
            ...
            self.candidate_set = sorted(
                self.candidate_set, reverse=True, key=lambda x: len(x)
            )
            seen_candidates = {}
    ...
    def top_n_candidates(
        ...
        **kwargs,
        ...
        self.embed_candidates(
            ...
            stemmer,
            "MaskAll"
            if ("cand_mode" not in kwargs or kwargs["cand_mode"] == "")
            else kwargs["cand_mode"],
        ...
        candidate_score = sorted(
            [(self.candidate_set[i], 1.0 - doc_sim[i][0]) for i in range(len(doc_sim))],
            reverse=True,
            key=lambda x: x[1],
        )
