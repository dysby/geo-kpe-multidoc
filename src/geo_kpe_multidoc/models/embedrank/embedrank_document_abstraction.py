...
class Document:
        ...
        if not memory:
            ...
            for i in range(len(self.doc_sents_words)):
                self.doc_sents_words_embed.append(
                    model.embed(stemmer.stem(self.doc_sents_words[i]))
                    if stemmer
                    else model.embed(self.doc_sents_words[i])
                )
        else:
    ...
    def embed_candidates(
        ...
        for candidate in self.candidate_set:
            ...
            for mention in self.candidate_mentions[candidate]:
                ...
                for i in range(len(self.doc_token_ids)):
                    if (
                        filt_ids[0] == self.doc_token_ids[i]
                        and filt_ids == self.doc_token_ids[i : i + cand_len]
                    ):
                        candidate_embeds.append(
                            np.mean(
                                self.doc_token_embeddings[i : i + cand_len]
                                .detach()
                                .numpy(),
                                0,
                            )
        ...
        if "z_score" in post_processing:
            self.candidate_set_embed = z_score_normalization(
                self.candidate_set_embed, self.raw_text, model
            )

    def extract_candidates(
        self, min_len: int = 5, grammar: str = "", lemmer: Callable = None
    ):
        """
    ...
    def evaluate_n_candidates(self, candidate_set_embed, candidate_set) -> List[Tuple]:
        doc_sim = np.absolute(
            cosine_similarity(candidate_set_embed, self.doc_embed.reshape(1, -1))
        )
    ...
    def top_n_candidates(
        ...
        **kwargs,
        ...
        post_processing = (
            [""] if "post_processing" not in kwargs else kwargs["post_processing"]
        )
        ...
        if "MMR" not in kwargs:
            doc_sim = np.absolute(
                cosine_similarity(
                    self.candidate_set_embed, self.doc_embed.reshape(1, -1)
                )
            )
        else:
            n = len(self.candidate_set) if len(self.candidate_set) < top_n else top_n
            doc_sim = mmr(
                self.doc_embed.reshape(1, -1),
                self.candidate_set_embed,
                self.candidate_set,
                n,
                kwargs["MMR"],
        ...
        candidate_score = sorted(
            [(self.candidate_set[i], doc_sim[i][0]) for i in range(len(doc_sim))],
            reverse=True,
            key=lambda x: x[1],
        )
