...
from geo_kpe_multidoc import GEO_KPE_MULTIDOC_DATA_PATH, MAPBOX_TOKEN, GEO_KPE_MULTIDOC_MORDECAI_ES_URL


def process_MKDUC01():
	parser = Geoparser(es_hosts=GEO_KPE_MULTIDOC_MORDECAI_ES_URL)

	with open(path.join(GEO_KPE_MULTIDOC_DATA_PATH, 'MKDUC-01/MKDUC01.json'), 'r') as s_json:
	...
	with open(path.join(GEO_KPE_MULTIDOC_DATA_PATH, "MKDUC-01/MKDUC01-mordecai.json"), 'w') as d_json:
		json.dump(res, d_json, indent=4, separators=(',', ': '))

def build_map():
	with open(path.join(GEO_KPE_MULTIDOC_DATA_PATH,"MKDUC-01/MKDUC01-mordecai.json"), 'r') as s_json:
		...
		with open(path.join(GEO_KPE_MULTIDOC_DATA_PATH,"MKDUC-01/MKDUC01-geo_locations.json"), 'w') as d_json:
			json.dump(res, d_json, indent=4, separators=(',', ': '))
