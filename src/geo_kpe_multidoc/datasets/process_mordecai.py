...
def process_MKDUC01():
    ...
    with open(
        path.join(GEO_KPE_MULTIDOC_DATA_PATH, "MKDUC-01/MKDUC01.json"), "r"
    ) as s_json:
        ...
        for doc_group in docs:
            ...
            for doc in docs[doc_group]["documents"]:
                res[doc_group][doc] = str(
                    parser.geoparse(docs[doc_group]["documents"][doc])
                )

    with open(
        path.join(GEO_KPE_MULTIDOC_DATA_PATH, "MKDUC-01/MKDUC01-mordecai.json"), "w"
    ) as d_json:
...
def build_map():
    with open(
        path.join(GEO_KPE_MULTIDOC_DATA_PATH, "MKDUC-01/MKDUC01-mordecai.json"), "r"
    ) as s_json:
        ...
        with open(
            path.join(
                GEO_KPE_MULTIDOC_DATA_PATH, "MKDUC-01/MKDUC01-geo_locations.json"
            ),
            "w",
        ) as d_json:
            json.dump(res, d_json, indent=4, separators=(",", ": "))
