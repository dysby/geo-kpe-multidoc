...
class MKDUC01_Eval:
    def __init__(self, data_filepath):
        ...
        # = os.path.join(
        #     os.path.dirname(os.path.realpath(__file__)), "data", "MKDUC01.json"
        # )
        try:
    ...
    def get_topic_docs(self):
        ...
        return {
            topicId: [
                docTxt for docId, docTxt in self.data[topicId]["documents"].items()
            ]
    ...
    def evaluate(
        ...
        """
        return self.__f1AtKAllTopics(
            pred_kps,
            cutoffs=pred_trunc,
            cutGold=(20 if gold_trunc20 else 0),
            clusterLevel=clusterLevel,
        )

    def __f1AtK_clusters(
        self, kpsPred, kpsGoldClusters, cutoffs=[1, 5, 10, 15, 20], cutGold=0
    ):
        """
        ...
        # stemmed KPs:
        kpsPredStem = [
            " ".join([self.stemmer.stem(t) for t in kp.split()]) for kp in kpsPred
        ]
        kpsGoldClustersStem = [
            [" ".join([self.stemmer.stem(t) for t in kp.split()]) for kp in kpCluster]
            for kpCluster in kpsGoldClusters
        ]
        ...
        for cutoff in cutoffs:
            ...
            kpsUniquePredStems = list(
                concat([kp.split() for kp in list(set(kpsPredStemUse))])
            )
            ...
            for goldCluster in kpsGoldClustersStemUse:
                clusterTokens = list(
                    set(list(concat([kp.split() for kp in goldCluster])))
                )  # keep unique tokens in the cluster
            ...
            stemMatchCount = len(
                list(
                    (
                        Counter(kpsUniquePredStems) & Counter(kpsUniqueGoldStems)
                    ).elements()
                )
            )
    ...
    def __f1AtK(self, kpsPred, kpsGold, cutoffs=[1, 5, 10, 15, 20], cutGold=0):
        ...
        kpsGoldStem = [
            " ".join([self.stemmer.stem(t) for t in kp.split()]) for kp in kpsGold
        ]
    ...
    def __f1AtKForTopic(
        self, topicId, kpsPred, cutoffs=[1, 5, 10, 15, 20], cutGold=0, clusterLevel=True
    ):
        if clusterLevel:
            return self.__f1AtK_clusters(
                kpsPred,
                self.data[topicId]["keyphrases"],
                cutoffs=cutoffs,
                cutGold=cutGold,
            )
        else:
            kpsGold = [
                kp[0] for kp in self.data[topicId]["keyphrases"]
            ]  # use only the first KP of each cluster
            return self.__f1AtK(kpsPred, kpsGold, cutoffs=cutoffs, cutGold=cutGold)

    def __f1AtKAllTopics(
        self, kpsPredPerTopic, cutoffs=[1, 5, 10, 15, 20], cutGold=0, clusterLevel=True
    ):
        # get the scores per topic:
        scoresPerTopic = {}
        for topicId in self.data:
            if topicId in kpsPredPerTopic:
                scoresPerTopic[topicId] = self.__f1AtKForTopic(
                    topicId,
                    kpsPredPerTopic[topicId],
                    cutoffs=cutoffs,
                    cutGold=cutGold,
                    clusterLevel=clusterLevel,
                )
        ...
        for metric in metrics:
            overallScores[metric] = mean(
                [scoresPerTopic[topicId][metric] for topicId in scoresPerTopic]
            )
